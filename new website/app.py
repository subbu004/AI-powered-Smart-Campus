...
def staff_login():
    if request.method == "POST":
        ...
        cur.execute(
            "SELECT id, name,email,password FROM staff WHERE email=%s AND password=%s",
            (email, password)
        )
        staff = cur.fetchone()
        ...
        if staff:
            ...
            session["staff_password"]=staff[3]
            if (session["staff_email"]=="admin@americancollege.edu" and session["staff_password"]=="admin123"):
